 ...
 * is valid to say that this value is related to the node that the list represents, and not the list it-self.
...
/**
 * The function responsably for passing information from a generic BTree node to one partition of the lists in
 * the block passed as a parameter. The gap for the stored information is determinated by the positions and,
 * using they as parameters, we can apply this implementation in the differents splits and redistribuition.
 */
...
/**
 * Atualizes the information in a node according to one specific interval in a overflow block. The interval is determinated by
 * the inicial id and the information limit. The size limit represents the size of the key vector allocated in the node, so that,
...
/**
 * Creates and Overflow_block according to the situation involving three nodes, such as the redistribuition and the split
 * 2-3. This function allocates the memory dynamically and returns the list with the nodes, the father and the inserted
 * information to the program.
 * Is valid to say that this case do not calculates the ascending positions. That information has to be obtained outside the
 * function.
 */
...
/**
 * Follows the same logic of the last one, now, applying the general purpose functions to the situation of an unique single
 * node defining the overflow block. This situation describes the split 1 - 2, on account of your single application, it calculates
 * the asceding position inside the function,  unlike the previous one.
 */
...
// ------------------------------
/**
* Returns the sister page of the overflowed one, indicating the direction of the spliting
*  and storing its relative position in the Path_running adress (direction) passed as a parameter.
*/
