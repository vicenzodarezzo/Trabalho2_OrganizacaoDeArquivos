...
long long int header_bin_write(FILE * binFile, Header_t * header){
    ...
    counter += sizeof(long long int) * fwrite(&(header->proxByteOffset), sizeof(long long int), 1, binFile);
...
long long int header_reading(FILE * read_file, Header_t * header){
    
    assert(read_file);
    long long int counter = 0 ;
    counter += sizeof(char) * fread(&(header->status), sizeof(char), 1, read_file);
    counter += sizeof(long long int) * fread(&(header->proxByteOffset), sizeof(long long int), 1, read_file);
...
char * variable_field_reading(FILE * read_file, long long int * counter){
